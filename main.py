...
@login_required
@app.route("/map/<map_name>/delete")
def delete_map(map_name):
    db_sess = db_session.create_session()

    possible_map = (
        db_sess.query(Map)
        .filter(Map.user_id == current_user.id, Map.map_name == map_name)
        .first()
    )
    if possible_map:
        db_sess.delete(possible_map)

        path_to_maps = os.path.join(app.root_path, "maps", str(current_user.id))
        os.remove(os.path.join(path_to_maps, str(possible_map.map_file) + ".sgb"))

        if os.path.exists(
            os.path.join(path_to_maps, str(possible_map.map_file) + ".tga")
        ):
            os.remove(os.path.join(path_to_maps, str(possible_map.map_file) + ".tga"))
    else:
        abort(404)
    ...
    if form.validate_on_submit():
        ...
        if filename_sgb in DEFAULT_NAMES:
            return render_template(
                ...
                title="Upload map",
                form=form,
                header="Dummy, we Already have That One",
            )
        ...
        possible_map = (
            db_sess.query(Map)
            .filter(Map.user_id == current_user.id, Map.map_name == form.name.data)
            .first()
        )
        if possible_map:
            ...
            new_map = Map(
                map_name=form.name.data, map_file=filename_sgb, user_id=current_user.id
            )
            db_sess.add(new_map)
...
def sing_up():
    ...
    if form.validate_on_submit():
        ...
        if db_sess.query(User).filter(User.login == form.name.data).first():
            return render_template(
                ...
                header="Holy Emperror! Such account already Exists",
            )
    ...
    return render_template(
        "form.html",
        title="Sing up",
        header="Imperium Registration Service",
        form=form,
        enctype="application/x-www-form-urlencoded",
    )
...
def index():
    ...
    if current_user.is_authenticated:
        ...
        maps.extend(
            map(
                lambda x: (x.map_file, x.map_name, x.user_id),
                db_sess.query(Map).filter(Map.user_id == current_user.id),
            )
        )

    return render_template(
        "index.html", title="DoW Map Viewer", maps=maps, show_map_menu=True
    )
